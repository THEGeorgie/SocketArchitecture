...
void *get_in_addr(struct sockaddr *sa) //sockaddr describes a socket address (Address family(Ipv4,Ipv6 etc.) and address)
{
...
int main(void)
{
	...
	for(p = servinfo; p != NULL; p = p->ai_next) {
		...
		if (bind(sockfd, p->ai_addr, p->ai_addrlen) == -1) { //binds the socket to the adress
			close(sockfd);
